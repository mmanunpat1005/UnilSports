...
server <- function(input, output, session) {
  ...
  observe({
   ...
   output$selected_var <- renderTable(width = "auto", hover = T,{
     
     data.frame(optimize_output()[2]) %>%
       select(-c(table_result.time, table_result.METs, table_result.p)) %>% 
       rename(Date = table_result.Date, `Start Time` = table_result.Start.time,
              `End Time` = table_result.End.time,
              `Duration (min)` = table_result.Duration_min,
...
calburn <- 300
date <- c('2022-12-14')
activity <- c('Tango argentin / Débutants')
weight <- 50
time <- c('19:00 – 20:00', '20:00 – 21:00' )
optimize_output <- optimize_schedule(cleanschedule, date, activity, time, calburn, weight)




test <- data.frame(optimize_output[2]) %>%
      select(-c(table_result.Date, table_result.time, table_result.METs, table_result.p)) %>% 
      rename( `Start Time` = table_result.Start.time,
             `End Time` = table_result.End.time,
             `Duration (min)` = table_result.Duration_min,
...
test.2 <- data.frame(optimize_output[3]) %>%
  select(-c(activity_selected.Date, activity_selected.time, 
            activity_selected.METs, activity_selected.p)) %>% 
  rename(`Start Time` = activity_selected.Start.time,
         `End Time` = activity_selected.End.time,
         `Duration (min)` = activity_selected.Duration_min,
